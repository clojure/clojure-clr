...
;;;     I did preserve the original code behind  ;;; comments and I did mark my additions with ;;;DM;;;
;;; I put more of the support into C# rather than in Clojure, just so I could bang out the code quicker.
;;  This should be redone eventually.
    ...
    (apply str (.Replace (str *ns*) \- \_) ".proxy"                         ;;; .replace
      (interleave (repeat "$")
        (concat
          ...
          [(.ToString ^int (hash inames) "X")] [(clojure.lang.Compiler/IsCompilingSuffix)])))))                             ;;;[(Integer/toHexString (hash inames))])))))   Added type hint                
...
;;;                                        ;add methods matching supers', if 

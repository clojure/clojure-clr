...
(def ^:private pp-newline (memoize #(Environment/NewLine)))   ;;; (System/getProperty "line.separator")

(declare emit-nl)

(defmulti ^{:private true} write-token #(:type-tag %2))
(defmethod write-token :start-block-t [^TextWriter this token]
...
(defn- tokens-fit? [^TextWriter this tokens]
...
(defn- miser-nl? [^TextWriter this lb section]
...
(defn- write-line [^TextWriter this]
  (dosync
...
;;; writing
(defn- add-to-buffer [^TextWriter this token]
;  (prlabel a2b token)
  (dosync
...
(defn- write-buffered-output [^TextWriter this]
  (write-line this)
...
(defn- end-block [^TextWriter this]
  (dosync
...
(defn- nl [^TextWriter this type]
  (dosync 
   (setf :mode :buffering)
   (let [pos (getf :pos)]
     (add-to-buffer this (make-nl-t type (getf :logical-blocks) pos pos)))))

(defn- indent [^TextWriter this relative-to offset]
  (dosync 
   (let [lb (getf :logical-blocks)]
     (if (= (getf :mode) :writing)
       (do
...
(defn- get-miser-width [^TextWriter this]
  (getf :miser-width))

(defn- set-miser-width [^TextWriter this new-miser-width]
  (dosync (setf :miser-width new-miser-width)))

(defn- set-logical-block-callback [^TextWriter this f]

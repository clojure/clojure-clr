...
(defmacro when-let
  ...
  (assert-args
     (vector? bindings) "a vector for its binding"
     (== 2 (count bindings)) "exactly 2 forms in binding vector") ;; JVM has only =, I have a boxed type problem here.
   (let [form (bindings 0) tst (bindings 1)]
    `(let [temp# ~tst]
       (when temp#
         (let [~form temp#]
...
(defn set-agent-send-executor!
  ...
  {:added "1.5"}
  [executor]
  )                                                              ;;; No CLR equivalent.  make a no-op.   (set! clojure.lang.Agent/pooledExecutor executor))

(defn set-agent-send-off-executor!
  "Sets the ExecutorService to be used by send-off"
  {:added "1.5"}
  [executor]
  )                                                              ;;; No CLR equivalent.  make a no-op.   (set! clojure.lang.Agent/soloExecutor executor))
...
(defn send-via
  ...
  (.dispatch a (binding [*agent* a] (binding-conveyor-fn f)) args false))    ;;; No CLR equivalent.  Just use the thread pool. -- (.dispatch a (binding [*agent* a] (binding-conveyor-fn f)) args executor))
...
(defn send
  ...
  (.dispatch a (binding [*agent* a] (binding-conveyor-fn f)) args false))           ;;;  No CLR equivalent. keep old true/false style -- (apply send-via clojure.lang.Agent/pooledExecutor a f args))
...
(defn send-off
  ...
  {:added "1.0"
   :static true}
  [^clojure.lang.Agent a f & args]
  (.dispatch a (binding [*agent* a] (binding-conveyor-fn f)) args true))             ;;;  No CLR equivalent. keep old true/false style -- (apply send-via clojure.lang.Agent/soloExecutor a f args))
...
(defn class
  ...
  {:added "1.0"
   :static true}
  ^Type [^Object x] (if (nil? x) x (. x (GetType))))  ;;; Class getClass
  ...
  (cond
    ...
    (symbol? (bindings 0)) `(let ~(subvec bindings 0 2)
                              (try
                                ...
                                (finally
                                  (. ~(with-meta (bindings 0) {:tag 'IDisposable}) Dispose))))      ;;; close => Dispose
  ...
  (let [gmap (gensym "map__")
        ...
        xf (fn [mk]
             (let [mkns (namespace mk)
                   mkn (name mk)]
               (cond (.StartsWith mkn "keys") #(keyword (or mkns (namespace %)) (name %))                      ;;; .startsWith
                     (.StartsWith mkn "syms") #(list `quote (symbol (or mkns (namespace %)) (name %)))         ;;; .startsWith
                     (.StartsWith mkn "strs") str                                                              ;;; .startsWith
                        ...
                        (let [bb (first bbs)]
                            (if (= bb '&)
                              (if preamp?
                                ...
                                (throw (new ArgumentException (str "& can only appear once in " dir))))                     ;;; IllegalArgumentException
                              (let [_
...
(defn ex-message
  ...
  (when (instance? Exception ex)                                   ;;; Throwable
    (.Message ^Exception ex)))                                     ;;; .getMessage Throwable
...
(defmacro assert
  ...
  ([x]
     (when *assert*
       `(when-not ~x
          (throw (new Exception (str "Assert failed: " (pr-str '~x)))))))                        ;;; AssertionError
  ([x message]
     (when *assert*
       `(when-not ~x
    ...
    (if (> 2 (count clauses))
      ...
      (let [pairs (partition 2 clauses)
            assoc-test (fn assoc-test [m test expr]
                         (if (contains? m test)
                           (throw (ArgumentException. (str "Duplicate case test constant: " test)))     ;;; IllegalArgumentException
                           (assoc m test expr)))
            pairs (reduce1
                       (fn [m [test expr]]
                         (if (seq? test)
            ...
            thens (vals pairs)
            mode (cond
                   (every? #(and (integer? %) (not (char? %)) (<= Int32/MinValue % Int32/MaxValue)) tests)          ;;; Integer/MIN_VALUE   Integer/MAX_VALUE  - (not (char? %)) added to match char-handling semantics of JVM
                   :ints
                   (every? keyword? tests)
                   :identity
                   :else :hashes)]
  ...
  (let [d (clojure.lang.CountDownLatch. 1)                       ;;; java.util.concurrent.CountDownLatch.
    ...
    (reify 
     ...
     clojure.lang.IBlockingDeref
       (deref
        [_ timeout-ms timeout-val]
        (if (.Await d timeout-ms)                                     ;;; .Await  java.util.concurrent.TimeUnit/MILLISECONDS
          @v
          timeout-val))  
     clojure.lang.IPending
      (isRealized [this]
...
(defn- ^{:dynamic true} assert-valid-fdecl
  ...
  [fdecl]
  (when (empty? fdecl) (throw (ArgumentException.                                          ;;; IllegalArgumentException
...
(defn reader-conditional
  ...
  {:added "1.7"}
  [form splicing?]                                          ;;; removed ^Boolean on splicing?

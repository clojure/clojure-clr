...
(defn- cli-build
  ...
  (or @build
    (let [result (try
                   #?(:clj (proc/exec "clojure" "--version") :cljr  (proc/exec {:redirect-out true} "powershell.exe" "cljr --version"))
                   (catch Exception e ""))
      ...
      (when version                                                                                                                           ;;; this will always be false.  We don't have BUILD in our version.
...
(defn- validate-version                                                                                                                       
  ...
  (if version
    (when (< version 1347)
      (throw (#?(:clj RuntimeException. :cljr Exception.) "Clojure CLI version is older than minimum required version, 1.11.1.1347. Please update to latest version.")))
...
(defn ^:dynamic invoke-tool
  ...
  (let [args (conj [fn] (assoc args :clojure.exec/invoke :fn))
        ...
        command-strs [#?(:clj "clojure" :cljr "cljr") (str "-T" (or tool-alias tool-name)) "-"]
        _ (when (:debug opts) (apply println "Invoking: " command-strs))
        proc #?(:clj (apply proc/start command-strs)  :cljr (apply proc/start {:redirect-in true :redirect-out true :redirect-err true} command-strs) )
        in (proc/stdin proc)
    ...
    (binding [*print-length* nil
      ...
      #?(:clj 
         ...
         :cljr
         (proc/io-task
           #(with-open [w (cio/text-writer in)]
              (doseq [a args]
                (.Write w (pr-str a))
                (.Write w " "))))))

...
(ns clojure.repl.deps
  ...
  (:require
   ...
   #?(:clj [clojure.java.basis :as basis] :cljr [clojure.clr.basis :as basis])
   #?(:clj [clojure.java.basis.impl :as basis-impl] :cljr [clojure.clr.basis.impl :as basis-impl])
   [clojure.tools.deps.interop :as tool])
  (:import
    #?(:clj [clojure.lang DynamicClassLoader])
    #?(:clj [java.io File]) ))
...
(defn add-libs
  ...
  {:added "1.12"}
  [lib-coords]
  (when-not *repl* (throw (#?(:clj RuntimeException. :cljr Exception.) "add-libs is only available at the REPL")))
  (let [{:keys [libs] :as basis} (basis/current-basis)
    ...
    (when-not (empty? lib-coords)
      (let [procurer (dissoc basis [:basis-config :paths :deps :aliases :argmap :classpath :classpath-roots])
            ...
            #?(:clj urls) #?(:clj (->> paths (map jio/file) (map #(.toURL ^File %))))]
        #?(:clj (run! add-loader-url urls))

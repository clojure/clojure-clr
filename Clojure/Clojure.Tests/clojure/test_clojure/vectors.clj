...
(deftest test-subvec
  (let [v1 (vec (range 100))
        v2 (subvec v1 50 57)]
    (is (thrown? ArgumentOutOfRangeException (v2 -1)))            ;;; IndexOutOfBoundsException
    (is (thrown? ArgumentOutOfRangeException (v2 7)))             ;;; IndexOutOfBoundsException
    (is (= (v1 50) (v2 0)))
    (is (= (v1 56) (v2 6)))))
...
(deftest test-vector-eqv-to-non-counted-types
  ...
  (is (= [0 1 2] (new System.Collections.ArrayList [0 1 2])))                 ;;; java.util.ArrayList
...
#_(deftest test-empty-vector-spliterator
  (let [v []
...
#_(deftest test-spliterator-tryadvance-then-forEach
  (let [n 66
...
#_(deftest test-spliterator-trySplit
  (dotimes [n 257]
    (let [v (vec (range n))
          seen (atom #{})
          consumer (remember seen)
...
#_(deftest test-vector-parallel-stream
  (dotimes [n 1024]
    (let [v (vec (range n))]
      (is (= n

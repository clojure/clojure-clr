...
(deftest protocols-test
  ...
  (testing "protocol fns throw IllegalArgumentException if no impl matches"
    (is (thrown-with-msg?
          ...
          #"No implementation of method: :foo of protocol: #'clojure.test-clojure.protocols.examples/ExampleProtocol found for class: Int64"  ;;; java.lang.Long
          (foo 10))))
...
(deftest degenerate-defrecord-test
  (let [empty (EmptyRecord.)]
    (is (nil? (seq empty)))
    (is (not (.containsKey empty :a)))))             ;;; containsValue
...
(deftest defrecord-factory-fns
  ...
  {:xxx 42}))))
      ...
      (testing "record equality"
        ...
        (is (= r-n (map->RecordToTestFactories (System.Collections.Hashtable.))))                  ;;; (java.util.HashMap.)
  ...
  (testing "methods can recur"
    (let [r (reify
             ...
             (get_Item [_ index]                                     ;;; get
                  (if (zero? index)
                    :done
                    (recur (dec index)))))]           ;;; int cast added

...
(defn- possible-exception? [ex]
       (or (instance? ArgumentException ex)           ;;; IllegalArgumentException
           (instance? InvalidCastException ex)        ;;; ClassCastException
           (instance? NullReferenceException ex)))    ;;; NullPointerException

(defmacro return-exc [& forms]
  `(try ~@forms (catch Exception e# e#)))           ;;; Throwable
...
(deftest test-transduce
  (let [long+ (fn ([a b] (+ (long a) (long b)))
        ...
        int-array (into-array Int32 (map #(int %) arange))         ;;; Integer/TYPE  #(Integer. (int %))

...
(simple-tests angle-bracket-max-column-tests
  (cl-format nil "~%;; ~{~<~%;; ~1,50:; ~A~>~}.~%" (into [] (split "This function computes the circular thermodynamic coefficient of the thrombulator angle for use in determining the reaction distance" #"\s" )))	;;RJ: Updated .Split to clojure.string/split and move Regex as the second function parameter for correct function behavior
...
(def big-pos-ratio (/ (* 4 (bigint (BigDecimal/Create Double/MaxValue))) 3))                                        ;;; (. BigDecimal valueOf Double/MAX_VALUE)
...
(def tiny-neg-ratio (- tiny-pos-ratio))
 ...
 (simple-tests cltl-F-tests
  ...
  (binding [*math-context* clojure.lang.BigDecimal+Context/Decimal128]            ;;; java.math.MathContext/DECIMAL128  They have 2396924179816420933333333333333333  - problem in BigDecimal?  Maybe JVM                                               
    (cl-format false "~10,3f" big-pos-ratio)) ".000"
  (binding [*math-context* clojure.lang.BigDecimal+Context/Decimal128]            ;;; java.math.MathContext/DECIMAL128  They have 2396924179816420933333333333333333  - problem in BigDecimal?  Maybe JVM 
    (cl-format false "~10,3f" big-neg-ratio)) "-.000"
  (binding [*math-context* clojure.lang.BigDecimal+Context/Decimal128]            ;;; java.math.MathContext/DECIMAL128

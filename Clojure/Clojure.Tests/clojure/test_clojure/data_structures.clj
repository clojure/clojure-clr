...
(ns clojure.test-clojure.data-structures
  ...
  (:import [System.Collections ICollection]))                       ;;; java.util Collection
...
(deftest test-sorted-map-keys
  (is (thrown? InvalidCastException (sorted-map () 1)))              ;;; ClassCastException
  (is (thrown? InvalidCastException (sorted-map #{} 1)))             ;;; ClassCastException
  (is (thrown? InvalidCastException (sorted-map {} 1)))              ;;; ClassCastException

  (is (thrown? InvalidCastException (assoc (sorted-map) () 1)))      ;;; ClassCastException
  (is (thrown? InvalidCastException (assoc (sorted-map) #{} 1)))     ;;; ClassCastException
  (is (thrown? InvalidCastException (assoc (sorted-map) {} 1)))      ;;; ClassCastException
...
(deftest test-map-entry?
  ...
  #_(testing "map-entry? = true"
    (are [entry]
      (true? (map-entry? entry))
      #_(first (doto (System.Collections.Hashtable.) (.Add "x" 1))) )))                  ;;; (doto (java.util.HashMap.) (.put "x" 1))  -- we don't cover this case with map-entry?  Maybe we should'
...
(deftest test-sorted-set
  ...
  (is (thrown? InvalidCastException (sorted-set ())))                             ;;; ClassCastException
  (is (thrown? InvalidCastException (sorted-set {})))                             ;;; ClassCastException
  (is (thrown? InvalidCastException (sorted-set #{})))                            ;;; ClassCastException
...
(deftest test-assoc
  ...
  (let [msg (format "(class a)=%s (class b)=%s a=%s b=%s"
    ...
    (when (instance? ICollection a)                                         ;;; Collection
      (is (= (count a) (.get_Count a)) msg))                                ;;; .size 
    (when (instance? ICollection b)                                         ;;; Collection
      (is (= (count b) (.get_Count b)) msg))                                ;;; .size 
    (is (= a b) msg)
    (is (= b a) msg)

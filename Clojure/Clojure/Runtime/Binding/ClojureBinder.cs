...
namespace clojure.lang.Runtime.Binding
{
    ...
    public class ClojureBinder : DefaultBinder
    {
        ...
        public override Ast ConvertExpression(Ast expr, Type toType, ConversionResultKind kind, OverloadResolverFactory resolverFactory)
        {
            ...
            {
                Type fromType;
                // We have a little problem with DLR internal calling wrapping an extra Convert(x,typeof(Object))
                // This is due to Microsoft.Scripting.Runtime.MetaObjectExtensions.Restrict looking for a visible type.
                // On types generated by, e.g., System.Linq.Enumerable.Range, the type is not visible, so it converts to the visible base, namely Object.
                UnaryExpression uexpr = expr as UnaryExpression;
                if (uexpr != null && uexpr.NodeType == ExpressionType.Convert && uexpr.Type == typeof(Object))
                    fromType = uexpr.Operand.Type;
                else
                    fromType = expr.Type;

                if (toType.IsAssignableFrom(fromType))
                    return uexpr.Operand;
                ...
                {
                    return MakeToGenericConversion(expr,toType,fromType,typeof(ListGenericWrapper<>));
                }
                else if (genTo == typeof(IDictionary<,>))
                {
                    return MakeToGenericConversion(expr,toType,fromType,typeof(DictionaryGenericWrapper<,>));
                }
                else if (genTo == typeof(IEnumerable<>))
                {   
                    return MakeToGenericConversion(expr, toType, fromType,typeof(IEnumerableOfTWrapper<>));
                }
            }

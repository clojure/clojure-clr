...
namespace clojure.lang.CljCompiler.Ast
{
    public class CaseExpr : Expr, MaybePrimitiveExpr
    {
        ...
        public void DoEmit(RHC rhc, ObjExpr objx, CljILGen ilg, bool emitUnboxed)
        {
            ...
            {
                Label[] testLabels = new Label[thenLabels.Count];
                for (int i = 0; i < thenLabels.Count; i++)
                    testLabels[i] = ilg.DefineLabel();

                // The hash value to test is sitting on top of the stack.
                // We need to store it so that each test generated by EmitSparseCaseTests can load it.

                LocalBuilder hashLoc = ilg.DeclareLocal(typeof(int));
            ...
            foreach (int i in thenLabels.Keys)
            {
                ilg.MarkLabel(thenLabels[i]);
            ...
            Label defaultLabel)
        {
            ...
            // Test against the current key
            ilg.Emit(OpCodes.Ldloc, hashLoc); // load the hash value of the test expression
            ilg.EmitInt(thenValues[midIndex]);
            ilg.Emit(OpCodes.Beq, thenLabels[midIndex]);

            // Determine if we need to branch left or right
            if (leftIndex == rightIndex)
            {
                // No tests below us
                ilg.Emit(OpCodes.Br, defaultLabel);
            ...
            int newRight = midIndex - 1;
            int newLeft = midIndex + 1;

            bool leftExists = newRight >= leftIndex;
            bool rightExists = newLeft <= rightIndex;
